		...
		<div className="pointer-events-none absolute inset-0 select-none overflow-hidden">
			...
			{flash && (
				<>
					<div
						key={`label-${flash.id}`}
						className="animate-wolf-rise absolute top-[26%] left-1/2 -translate-x-1/2"
					>
...
function Segment({ value, unit }: { value: string; unit: string }) {
	return (
		<span className="relative inline-flex items-baseline">
			<span className="text-[7cqw] leading-none [text-shadow:0_0_2.4cqw_rgba(0,172,237,0.45)]">
				{value}
			</span>
			<span className="ml-[0.25cqw] font-heading text-[1.4cqw] font-bold text-[#5bc8f0]">
				{unit}
			</span>
		</span>
	);
