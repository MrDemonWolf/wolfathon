...
export function TimerPanel({ doc, onChanged }: { doc: TimerDoc | undefined; onChanged: () => void }) {
  ...
  const busy =
    start.isPending || pause.isPending || reset.isPending || addMinutes.isPending || applyEvent.isPending;
    ...
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          {[1, 5, 10, 30].map((min) => (
            <Button key={min} variant="outline" className="rounded-lg" onClick={() => addMinutes.mutate({ minutes: min })} disabled={busy}>
              +{min}m
            </Button>
          ))}
          <Button variant="outline" className="rounded-lg" onClick={() => addMinutes.mutate({ minutes: -5 })} disabled={busy}>
            −5m
          </Button>
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <Button variant="secondary" className="rounded-lg" onClick={() => applyEvent.mutate({ kind: "sub", tier: "t1" })} disabled={busy}>
            Sub T1
          </Button>
          <Button variant="secondary" className="rounded-lg" onClick={() => applyEvent.mutate({ kind: "sub", tier: "t2" })} disabled={busy}>
            Sub T2
          </Button>
          <Button variant="secondary" className="rounded-lg" onClick={() => applyEvent.mutate({ kind: "sub", tier: "t3" })} disabled={busy}>
            Sub T3
          </Button>
          <Button variant="secondary" className="rounded-lg" onClick={() => applyEvent.mutate({ kind: "gift", tier: "t1", count: 1 })} disabled={busy}>
            Gift ×1
          </Button>
          <Button variant="secondary" className="rounded-lg" onClick={() => applyEvent.mutate({ kind: "bits", bits: 100 })} disabled={busy}>
            100 bits
          </Button>
        </div>
      </div>
